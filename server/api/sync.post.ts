...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ worklogs: WorklogEntry[]; dateOffsetHours?: number }>(event)

  if (!body.worklogs?.length) {
    throw createError({ statusCode: 400, message: 'No worklogs provided' })
  }

  const results = await syncWorklogsToCostlocker(config, body.worklogs, body.dateOffsetHours ?? 0)
